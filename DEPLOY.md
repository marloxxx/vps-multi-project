...
`setup.sh` creates `.env` (domain + ACME email), generates random passwords, starts stack services (Traefik dashboard + Portainer included, monitoring enabled by default, **9Router off unless you opt in**), then configures **SSH on a random high port**, **ufw** (new port + 80 + 443 + 22 until you migrate), fail2ban, cron.
